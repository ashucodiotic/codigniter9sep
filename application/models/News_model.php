...
class News_model extends CI_Model {
	...
	public function get_all_news($page,$limit,$q=false){
	        ...
	        $query  = "SELECT  *  FROM news_tbl WHERE is_deleted = 0";
	    	if ($q){
	    		$query .= " AND news_title LIKE '%".$q."%' ";
	    	}
	        $query  .= " ORDER BY created_at DESC LIMIT ".$limit." OFFSET ".$offset;
			$rows   =  $this->db->query($query)->result_array();	

			//rows counting
			$query  = "SELECT  COUNT(*) count FROM news_tbl WHERE is_deleted = 0";
	       	if($q){
	        	$query .= " AND news_title LIKE '%".$q."%' ";
	        }
